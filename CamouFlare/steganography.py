...
class Steganography:
    ...
    def hide_data(
        self,
    ) -> None:  # Evoke encode method and create a new image instance with data encoded.
    ...
    def encode(self) -> None:  # encoding data into the image
        ...
        for data in binary_value:
            for i in range(0, 8):
                if (
                    data[i] == "0" and self.img[row, col, i % 3] % 2 != 0
                ):  # bgr value to be even, when bin value=0
                    self.img[row, col, i % 3] -= 1
                elif (
                    data[i] == "1" and self.img[row, col, i % 3] % 2 == 0
                ):  # bgr value to be odd, when bin value=1
            ...
            if current_data_len == self.data_len - 1:
                if (
                    self.img[row, col, 2] % 2 == 0
                ):  # if all values of the given data has been encoded, (r value)= odd value
    ...
    def decode(self) -> list:
        ...
        while True:
            for i in range(0, 8):
                if (
                    self.img[row, col, i % 3] % 2 == 0
                ):  # if bgr value is even, bin value=0
                    data += "0"
